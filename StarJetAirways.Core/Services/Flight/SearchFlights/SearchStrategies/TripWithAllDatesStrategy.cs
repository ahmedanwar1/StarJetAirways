...
public class TripWithAllDatesStrategy : IFlightsSearchStrategy
{
    private readonly IFlightsRepository _flightsRepository;

    public TripWithAllDatesStrategy(IFlightsRepository flightsRepository)
    {
        _flightsRepository = flightsRepository;
    }
    public async Task<IEnumerable<FlightResponseDTO>?> SearchFlightsAsync(FlightSearchCriteriaDTO searchCriteria)
    {
        var flights = await _flightsRepository.GetTripWithAllDatesFlightsAsync(searchCriteria);
